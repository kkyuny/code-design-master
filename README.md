...
## API Test 개선
- 테스트 시 json 관리의 불편
- 객체기반으로 생성해야하는 단점, 테스트 코드에서만 사용될 코드를 만드는 문제
    - resources 패키지 안에 .json 파일을 생성 후 테스트 코드에서 readJson()하여 해결
    - 객체 생성시 생성자를 AceessLevel.PRIVATE로 설정하면 불필요한 테스트 목적의 코드 생성을 방지할 수 있다. 
